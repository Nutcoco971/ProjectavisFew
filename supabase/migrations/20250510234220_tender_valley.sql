/*
  ...
  1. New Tables
    - `users`: Custom user data extending Supabase auth
    - `contents`: Stores all media content (films, series, music, podcasts)
    - `reviews`: Stores user reviews for content
    - `lists`: User-created content lists
    - `list_items`: Junction table for contents in lists
    - `tags`: Reusable tags for lists and content
    - `admin_queue`: Pending content for admin review
...
CREATE POLICY "Users can create reviews"
  ...
  WITH CHECK (
    NOT EXISTS (
      ...
      AND (r.user_id = auth.uid() OR r.anonymous_id = anonymous_id)
    )
  );
...
BEGIN
  IF EXISTS (
    ...
    AND (
      (r.user_id IS NOT NULL AND r.user_id = NEW.user_id)
      OR (r.anonymous_id IS NOT NULL AND r.anonymous_id = NEW.anonymous_id)
    )
  ) THEN
